...
export function SocialPartyLayout({ model }: Props) {
  ...
  const stageLabel = model.stage === "avatar" ? (en ? "Your character" : "Dein Charakter") : model.stage === "submit" ? (en ? "Your turn" : "Du bist dran") : model.stage === "vote" ? (en ? "Make your choice" : "Stimme ab") : model.stage === "finished" ? (en ? "Game over" : "Spiel vorbei") : (en ? "Watch the screen" : "Schau auf den Bildschirm");
  ...
  return <main className="social-party" key={model.resetKey}>
    <style>{`
      ...
      .sp-choice{display:flex;align-items:center;gap:14px}.sp-choice-avatar{width:54px;height:54px;flex:none;border-radius:50%;object-fit:cover;background:#e9d9cc;display:grid;place-items:center;font:800 24px system-ui,sans-serif;color:#4a354c}.sp-choice:disabled{opacity:.55}.sp-photo.is-avatar{aspect-ratio:1;max-width:320px;margin:auto;border-radius:50%;transform:none}
    ...
    <section className="sp-paper">{model.helperText && <p className="sp-helper">{model.helperText}</p>}
      {remainingMs !== null && model.stage === "submit" && <div style={{ marginTop: 16 }}><span className="sp-muted" aria-live="off">{en ? "Time left" : "Noch Zeit"}: {Math.ceil(remainingMs / 1000)} s</span><div className="sp-progress" style={{ marginTop: 6 }}><i style={{ width: `${progress}%` }} /></div></div>}
    </section>
    ...
    {model.stage === "submit" && !model.hasSubmitted && <section className="sp-actions" key={model.taskKind}>
      ...
      {model.taskKind === "draw" && <><div className="sp-toolbar">{INK.map((swatch) => <button type="button" key={swatch} className="sp-tool" aria-label={`${en ? "Color" : "Farbe"} ${swatch}`} aria-pressed={color === swatch} disabled={disabled} onClick={() => setColor(swatch)} style={{ borderColor: color === swatch ? swatch : undefined }}><span style={{ display: "inline-block", width: 14, height: 14, verticalAlign: "middle", borderRadius: 99, background: swatch, marginRight: 7 }} />{color === swatch ? (en ? "Selected" : "Aktiv") : ""}</button>)}{!model.basePhoto && <label className="sp-tool" role="button" tabIndex={disabled ? -1 : 0} onKeyDown={openFileWithKeyboard} style={{ cursor: "pointer" }}>{en ? "Add photo" : "Foto als Vorlage"}<input type="file" accept="image/*" disabled={disabled} hidden onChange={(e) => { const file = e.currentTarget.files?.[0]; e.currentTarget.value = ""; void onImage(file); }} /></label>}<button className="sp-tool" type="button" disabled={disabled || !strokes.length} onClick={() => setStrokes((v) => v.slice(0, -1))}>{en ? "Undo" : "Rückgängig"}</button><button className="sp-tool" type="button" disabled={disabled} onClick={() => { uploadIdRef.current += 1; setUploading(false); setStrokes([]); setPhoto(model.basePhoto ?? null); setPhotoReady(true); }}>{en ? "Clear strokes" : "Striche löschen"}</button></div>{model.maxStrokes && <p className="sp-caption" aria-live="polite">{strokes.length} / {model.maxStrokes} {en ? "strokes" : "Striche"}</p>}{imageError && <p className="sp-caption" role="alert">{en ? "Photo or drawing could not be compressed. Try a smaller image." : "Foto oder Zeichnung konnte nicht klein genug komprimiert werden."}</p>}<canvas ref={canvasRef} className="sp-canvas" width={640} height={480} aria-label={en ? "Drawing canvas" : "Zeichenfläche"} onPointerDown={(e) => { if (disabled || (model.maxStrokes && strokes.length >= model.maxStrokes)) return; e.currentTarget.setPointerCapture(e.pointerId); drawingRef.current = true; const p = point(e); setStrokes((current) => [...current, { color, points: [p] }]); }} onPointerMove={(e) => { if (drawingRef.current) addPoint(e); }} onPointerUp={() => { drawingRef.current = false; }} onPointerCancel={() => { drawingRef.current = false; }} /><button style={buttonStyle} type="button" disabled={disabled || uploading || !strokes.length || !photoReady} onClick={submitDrawing}>{en ? "Send drawing" : "Zeichnung senden"}</button></>}
    </section>}
    ...
    {model.stage === "finished" && <p className="sp-helper">{en ? "The final scores are on the shared screen." : "Der Endstand ist auf dem Hauptbildschirm zu sehen."}</p>}

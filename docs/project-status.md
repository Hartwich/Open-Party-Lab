...
- Chaos-Kommando can be loaded from `local-games/chaos-kommando` when cloned locally. It is recommended alpha. Its host uses a continuous modular toasted-marshmallow rig with a fixed world pivot, aim-tracking eyes, procedural locomotion, weapon-specific hand grips, and dedicated carry art for all 16 weapons. The self-contained Marshmallow Motion Lab lives in the platform repository at `tools/marshmallow-motion-lab`, owns all torso, limb, weapon, headgear, and preview assets required by the tool, and exports reusable checked-in per-body-form rig profiles, a machine-readable motion handoff, and an implementation guide under `tools/marshmallow-motion-lab/presets`. Profiles include adjustable two-hand weapon reach and independent helmet/headband height and scale; six normalized headband colors and a separate 30-item comic arsenal are prepared as optional game asset sets. The arsenal includes four airstrike-marker alternatives, a proximity mine, and a supply crate; it is indexed for reuse but deliberately not added wholesale to the Motion Lab selector.

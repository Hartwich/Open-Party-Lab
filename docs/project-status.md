...
- Blickwinkel lives in `local-games/blickwinkel` as a separate game package for 1–16 players. Everyone starts with a character selfie. Its ten-task session has four person picks, two text tasks, two photo tasks, and two drawing tasks, including an assigned other-player selfie and stroke-limited prompts. The server runs an individual host showcase, a full host gallery that fits up to 16 entries at once, untimed name-and-avatar ballots for creative tasks, a current-score scene, and a countdown before the next task. Phones show the current input or ballot without repeating the task or creative entries. The host has animated result presentations and audio cues. Media remains transient in room state; real multi-phone, mobile-camera, and sound playback QA remain open.

...
- host settings store music and game sound-effect output volumes separately on the host device
...
- Dungeon-Gilde lives in `local-games/dungeon-guild` as an optional standalone game package with a public repository on `main`. It has separate Door and Treasure decks, private phone hands, visible levels/equipment, monster combat, help and reward sharing, escape with monster-specific penalties, room loot, charity, item selling and level-10-by-monster victory. Card names, text and visual design are original. Its portrait phone layout separates character/equipment from the hand, shows a readable selected card above a compact tappable hand, and keeps selling with the character's gear. The host uses compact equipment icon strips, an animated door reveal and generated original card illustrations; brief synthesized cues mark doors, combat and escape. Core game flow is implemented for 3–6 players; balance and real multi-phone validation remain open. Hosted builds clone it through the game catalog.
- Blickwinkel lives in `local-games/blickwinkel` as a separate game package for 1–16 players. Everyone starts with a character selfie. Its twelve-task session has four person picks, two text tasks, two photo tasks, and four drawing tasks: varied edits of another player's selfie and one's own, free drawing, and stroke-limited prompts. The server assigns selfie targets for the whole task and promotes submitted edits to those players' current character photos after voting; later tasks use the updated images. Individual host showcases lead to untimed voting: text answers show avatar and name beside the entry, while photo and drawing answers show the author's name above the image. Winning creative entries receive a short sound and firework reveal before scoring. The score scene places compact instant photos in one to three rows, drops coins over round winners, then colours the top three frames gold, silver, and bronze. The phone shows only the current input or name-and-avatar ballot, with icon drawing tools and no visible timer, helper copy, or score. Media remains transient in room state; real multi-phone, mobile-camera, and sound playback QA remain open.
...
- Card Table now includes Rommé: a 108-card double deck with four jokers, 13-card deals for 2–6 players, 40-point initial melding, later table extensions, stock recycling, a Rommé bot, and cumulative deadwood scoring.

...
- Schattenjagd can be loaded from `local-games/schattenjagd` when cloned locally. Its lobby also selects the per-turn time limit (20/30/45/60/90 seconds or no limit at all). It is playable alpha for 3–8 players: a procedurally generated three-layer transit network (taxi/bus/metro) is created per round, one hidden fugitive travels against a shared investigator team, only the used transport type is announced, and the fugitive surfaces on turns 3, 8, 13, 18, and 22. The host lobby selects whether the fugitive is a random player, a fixed seat number, or a server-side AI. All map data, artwork, naming, and rule values are original to that repo.

...
- Kartentisch can be loaded from `local-games/card-table` when cloned locally. It is playable alpha for 2–6 players and is the shared foundation for card games: deck presets (French 52, French 54 with jokers, German/Skat 32, a freely defined party deck, a 108-card double deck, a 60-card Wizard deck, an 80-card 1–20 deck), a server-side table engine with draw pile, discard pile, hands, free table zones and turn order, SVG card art rendered from code on both host and phone, and a landscape `card_hand` controller layout that renders whatever stacks a game declares. Five rulesets ship with it: Mau-Mau, Schwimmen (31), Wizard with bidding and trick play, Elfer raus with four colour rows, and an open free-play mode. A new card game is one `CardRuleset` implementation; the server runtime, the host table and the phone hand stay unchanged.
